...
HsInt z_grisu3(double v, char *buffer, HsInt *length, HsInt *d_exp)
{
...
HsInt z_grisu3_sp(float v, char *buffer, HsInt *length, HsInt *d_exp)
{
    int mk, kappa, success;
...
HsInt z_int_with(uint64_t x, HsInt sign, HsInt width, HsInt pad, char* ba, HsInt off)
{
    // writing from the right end
    char *start = ba + off;
    char *end = start + (width > 21 ? width : 21);
    char *p = end;
    char *q = start;
...
// Fast path for common case, sign is written at haskell side, no padding.
HsInt z_int(uint64_t x, char* ba, HsInt off)
{
    // writing from the right end
    char *start = ba + off;
    char *end = start + 21;
