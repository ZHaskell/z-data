{-|
Module      : Z.Data.FlatContainer.FlatMap
...
newtype FlatMap kv = FlatMap { sortedKeyValues :: V.Vector kv }
    deriving (Show, Eq, Ord, Typeable)

class Ord Key kv => KeyValPair kv where
    type Key kv :: Type
    type Val kv :: Type
    packKeyValPair :: (Key, Val) -> kv
    unpackKeyValPair :: kv -> (Key, Val)

instance KeyValPair (k, v) where
    type Key (k, v) = k
    type Key (k, v) = v
    packKeyValPair = id
    unpackKeyValPair = id

instance (KeyVal kv, T.Print (Key kv), T.Print (Val kv)) => T.Print (FlatMap k v) where
    {-# INLINE toUTF8BuilderP #-}
    toUTF8BuilderP p (FlatMap vec) = T.parenWhen (p > 10) $ do
        "FlatMap{"
        T.intercalateVec T.comma (\ kv ->
            case unpackKeyValue kv of (k, v) ->
                T.toUTF8Builder k >> T.char7 ':' >> T.toUTF8Builder v) vec
        T.char7 '}'

instance (KeyValPair kv, Arbitrary (Key kv), Arbitrary (Val kv)) => Arbitrary (FlatMap kv) where
    arbitrary = pack <$> arbitrary
    shrink v = pack <$> shrink (unpack v)

instance (KeyValPair kv, CoArbitrary (Key kv), CoArbitrary (Val kv)) => CoArbitrary (FlatMap kv) where
    coarbitrary = coarbitrary . unpack

instance Ord k => Semigroup.Semigroup (FlatMap kv) where
    {-# INLINE (<>) #-}
    (<>) = merge

instance Ord k => Monoid.Monoid (FlatMap kv) where
    {-# INLINE mappend #-}
    mappend = (<>)
    {-# INLINE mempty #-}
    mempty = empty

instance (NFData k, NFData v) => NFData (FlatMap kv) where
    {-# INLINE rnf #-}
