...
module Z.Data.Utils.Cast where
...
--
-- >>> cast (fromListN 3 [-1,-2,-3] :: PrimArray Int8) :: PrimArray Word8
-- fromListN 3 [255,254,253]
class Cast source destination where
    cast :: source -> destination

instance (Cast a b) => Cast (PrimArray a) (PrimArray b) where
    {-# INLINE cast #-}
    cast = unsafeCoerce#

instance (Cast a b) => Cast (MutablePrimArray s a) (MutablePrimArray s b) where
    {-# INLINE cast #-}
    cast = unsafeCoerce#
